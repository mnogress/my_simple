...
トレーニングデータでは完璧のスコアで学習をしたモデルで正解データで検証すると良くないスコアとなることを過学習(overfitting)と言います。通常トレーニングデータセットを更にトレーニン結果の評価用テストとトレーニングに細分化してそれぞれのデータでaccuracyのバラツキを見ることをクロスバリデーション(CV: cross-validatint)といいます。簡単に計算できます。
<!--more-->
...
上が85.0%でした。平均は84%です。　85%のトレーニング結果で、正解データで行うと81%の場合もありうるということです。この程度のバラツキが生じうることがわかります。

### パラメータの最適化はGridSearchで行う

トレーニングデータでのクロスバリデーションで的中率のバラツキの程度を把握できました。　ベストのパラメータはグリッドサーチテクニックと呼ばれるパラメータの総当り(Exhasutive)で決定します。　それは、次回のブログで説明いたします。ç
