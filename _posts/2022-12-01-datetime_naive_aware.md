...
datetime モジュールは、日付や時刻を操作するためのクラスで、日付や時刻に対する四則演算が可能です。日時のオブジェクトは、それらがタイムゾーンの情報を含んでいるかどうかによって "aware(タイムゾーン情報あり）" あるいは "naive（同無し）" に分類されます。

<!--more-->
このBlogでは、aware と naive の相互変換等の実践的な使い方についてTeamsライブイベントを例にまとめました。
