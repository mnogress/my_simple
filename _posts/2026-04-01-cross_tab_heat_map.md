...
<!--more-->
<p>
このページでは、Pandas と Seaborn を使って <strong>クロス集計表（crosstab）</strong> と <strong>ヒートマップ</strong> を作成し、カテゴリ同士の関係性を視覚的に理解する方法をまとめています。まず度数分布でデータの特徴を確認し、次にクロス集計表を作成し、最後にヒートマップで大小関係を可視化します。
列ごと・行ごとの正規化による比率比較まで扱うため、データ分析の基礎を体系的に学びたい方に最適です。
</p>
