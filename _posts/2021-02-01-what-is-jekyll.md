...
インストールできましたので、稼働確認をして、正常にインストールされたことを確認します。jkyll 実行コマンド(my_site フォルダにサイトイメージを出力するコマンド）を打って、ローカル環境でサーバの稼働を確認します。デフォルトの[minima](https://github.com/jekyll/minima){:target="_blank"}テーマが導入されます。
