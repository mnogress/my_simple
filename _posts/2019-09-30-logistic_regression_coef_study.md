...
<!--more-->
[「ロジスティック回帰分析＿その2」]({{"2019/09/20/logistic_regression_basic_2.html" | relative_url}}){:target="_blank"} では、各社員の退職の予想を二項ロジスティック回帰モデルにダミー変数を加えて的中率(hit ratio)をアップする手順を解説しました。 そのモデルを利用して説明します。
