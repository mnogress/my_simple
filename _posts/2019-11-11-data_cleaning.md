...
1. 入力ソースはEXCEL ファイルで、各月にシートは別れています。
2. CRM 同様に企業IDごとに活動をEXCELで集計し主にステージングを状況をPivotやグラフでまとめています
2. 従って、企業IDの重複は無い(あるべきではない）のに、それが1%弱あります。40,000のデータで重複は200～300程度あります
3. 各営業のテリトリー内の企業へのCall(訪問) の記録が重要は営業情報です。しかし、Call Date(訪問日)の入力間違いが少なからずあります 
4. 入力間違いはいろいろで、YYYY/MM/DD のところをHやRで始まるもの、「令和」と漢字入力されているもの、'YYYY/MM/DD予定'となっているもの、ありえない日にち 1930/12/22やなぜか2018./10/13 と'点'が入ってるのまで発見 
...
1. EXCELファイルの各シートをdf1, df2..とそれぞれデータフレームとして読み込みます
2. それぞれのデータフレームを連結して１つのデータフレームにします
3. pd.to_numericでEXCELのDateシリアルでない文字列のデータをNaNにし、その行を削除します
4. 分析期間でスライスします
5. 企業IDで重複排除します。その際、最新のエントリーを残します。 
...
CRMと同様、企業ID一つに対してその状況アップデートしていますので、同じ企業IDを持つ行の重複は無いはずですが、実際には重複があります。最新のデータを残して、重複処理をします。そのためには：
1. 日付で若い順にデータセットをソートします
2. `df.drop_duplicates([ keep='first' )`{:style="background-color: #ffe3e2; font-size: 0.9em"}で最初のデータを残すとします。
