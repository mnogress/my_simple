...
| `missing gem` のインストール<br>`Ruby` アプリの最初の起動時、必要な`gem`が無い場合に投入する。<br><br>`$ cd site_name`<br>`$ bundle exec jekyll serve`<br>で`missing gem`が出たら、このコマンドを投入する。 | `$ bundle install`{:style="background: #ffebf6"}<br><br>`Fetching gem metadata from https://rubygems.org/..........`<br>でインストールが開始する |
