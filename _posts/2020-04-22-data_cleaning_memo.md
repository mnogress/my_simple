...
サンプルコードでは220列ある元のデータセットに対して、列名だけを取り出して、１列 x 220行のデータフレームとして表示しています。
...
2. ブランクを取るのは主にテキストマイニングの際に必要となりますが、中には全角の空白（スペース）で単語を分けている場合がありますので、セットで半角スペースと全角スペースを取り除くことをお勧めします。
